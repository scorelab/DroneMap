...
var mongoose = require('mongoose')
   , TrackerLocations = mongoose.model('dronemap_collections')
...
exports.listLocationData = function (req, res) {
       TrackerLocations.find({}, 'location timestamp', {limit: 1, sort: {'timestamp': -1}}, function (err, data) {
            if (err) console.log(err)
            console.log(data);
            res.setHeader('content-type','application/json');
            res.send(data);
...
exports.listLocationsById = function(req, res){
  ...
  TrackerLocations.find({}, 'location', function(err, rcd){
    if(err) console.log(err);
    res.setHeader('content-type', 'application/json');
    res.send(rcd);
  });
